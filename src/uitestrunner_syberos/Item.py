...
class Item:
    ...
    def __refresh_attribute(self):
        ...
        self.__attributes["rotation"] = float(self.node.get("rotation"))
    ...
    def __init_attribute(self):
        ...
        self.__attributes["rotation"] = float()
    ...
    def rotation(self, refresh: bool = False) -> float:
        """
        获取元素控件的顺时针旋转角度。\n
        :param refresh: 是否刷新布局信息(默认值False)
        :return: 角度值
    ...
    def __xml_to_html_traverse(self, node: xml.dom.minidom.Element, index_z: int, x_offset: int, y_offset: int, root: bool = False):
        for child in node.childNodes:
            ...
            if float(child.getAttribute("opacity")) != 0 \
                ...
                rotation = child.getAttribute("relative_rotation")
                if int(child.getAttribute("hasContents")) == 0:
                    self.__html_string += "<div style=\"position:absolute;z-index:"+str(z)+";left:"+str(x)+"px;top:"+str(y)+"px;width:"+str(width)+"px;height:"+str(height)+"px;-webkit-transform:scale("+scale+") rotate("+rotation+"deg);"+clip+"\">"
                else:
                    if child.getAttribute("tempID") == self.temp_id():
                        self.__html_string += "<div style=\"position:absolute;z-index:"+str(z)+";left:"+str(x)+"px;top:"+str(y)+"px;width:"+str(width)+"px;height:"+str(height)+"px;-webkit-transform:scale("+scale+") rotate("+rotation+"deg);"+clip+"background:#F00;\">"
                    else:
                        self.__html_string += "<div style=\"position:absolute;z-index:"+str(z)+";left:"+str(x)+"px;top:"+str(y)+"px;width:"+str(width)+"px;height:"+str(height)+"px;-webkit-transform:scale("+scale+") rotate("+rotation+"deg);"+clip+"background:#00F;\">"
